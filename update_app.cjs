...
// 4. Remove the sizing code
// It looks like: {activeBottomPage === 'sizing' && ( ... )}
// I'll just use string replacement for the bulk of it.
code = code.replace(/{activeBottomPage === 'sizing' && \([\s\S]*?COMPLEMENTARY RETURNS ALLOWED FOR SIZING CORRECTION<\/span>\s*<\/div>\s*<\/div>\s*\)}/, "");
